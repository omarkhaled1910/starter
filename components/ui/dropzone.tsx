...
const DropzoneContent = ({
  ...
  console.log("onUploadFinalize", files, successes);
  useEffect(() => {
    if (successes.length > 0 && onUploadFinalize) {
      onUploadFinalize(successes);
    }
  }, [successes]);
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-y-2", className)}>
      ...
      <p className="text-sm">
        Upload{!!maxFiles && maxFiles > 1 ? ` Max ${maxFiles}` : ""} file
